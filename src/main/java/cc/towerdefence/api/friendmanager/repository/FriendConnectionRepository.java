...
public interface FriendConnectionRepository extends MongoRepository<FriendConnection, ObjectId> {
    ...
    @Query(value = "{$and: [{$or: [ {'playerOneId': ?0}, {'playerOneId': ?1} ]}, {$or: [ {'playerTwoId': ?0}, {'playerTwoId': ?1} ]}]}", exists = true)
    boolean areFriends(UUID playerOneId, UUID playerTwoId);

    // remove FriendConnection where a friend connection is mutual
    @Query(value = "{$and: [{$or: [ {'playerOneId': ?0}, {'playerOneId': ?1} ]}, {$or: [ {'playerTwoId': ?0}, {'playerTwoId': ?1} ]}]}", delete = true)
    long deleteByMutualIds(UUID playerOneId, UUID playerTwoId);
